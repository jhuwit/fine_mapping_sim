...
if(!dir.exists(here::here("data", "raw", "hemodynamics", "individual_timeseries"))){
  dir.create(here::here("data", "raw", "hemodynamics", "individual_timeseries"),
             recursive = TRUE)
}
...
# save to raw and to processed (since we're generating it ourselves, don't need to do any processing)
write_csv(eventtime_wide, here("data", "raw", "event_start_stop_times.csv"))


for(i in 1:nrow(eventtime_wide)){
  temp = eventtime_wide[i,]
  id = temp$id
  start = temp$`start_ADT_0-Anesthesia_0`
  ...
  df = 
    tibble(timestamp = ts,
           ...
           CVP = runif(n = length(ts), min = 0, max = 20),
           DBP = NA_real_,
           HR = NA_real_,
           MAP = runif(n = length(ts), min = 45, max = 115),
  ...
  prop_miss = runif(2, min = 0.05, max = 0.50)
...
covariates_analysis =
  tibble(id = seq(1:n),
         ...
         bin_akicomposite = rbernoulli(n, p = 0.1)* 1,
...
write_csv(covariates_analysis, here("data", "raw", "covariates.csv"))
